...
async def create_relationships(client, users, posts):
  ...
  for follower, followed in follows:
    await client.execute(
      f"RELATE {users[follower]}->follows->{users[followed]}"
    )
  ...
  for user, post in likes:
    await client.execute(
      f"RELATE {users[user]}->likes->{post}"
    )
...
async def query_direct_relationships(client, users):
  ...
  result = await client.execute(
    f"SELECT ->follows->user.* AS followed FROM {users['alice']}"
  )
  print("Alice follows:")
  ...
  # Who follows Alice?
  result = await client.execute(
    f"SELECT <-follows<-user.* AS followers FROM {users['alice']}"
  )
  print("\nAlice's followers:")
  if result and result[0].get('result'):
    for item in result[0]['result']:
...
async def query_post_interactions(client, users):
  ...
  result = await client.execute(
    f"SELECT ->likes->post.* AS posts FROM {users['alice']}"
  )
