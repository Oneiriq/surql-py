...
class Query(BaseModel, Generic[T]):
  ...
  def where(self, condition: str | Operator) -> 'Query[T]':
    ...
    return self.model_copy(update={
      'conditions': [*self.conditions, condition_str]
    })
  ...
  def order_by(self, field: str, direction: str = 'ASC') -> 'Query[T]':
    ...
    return self.model_copy(update={
      'order_fields': [*self.order_fields, (field, direction.upper())]
    })
  ...
  def group_by(self, *fields: str) -> 'Query[T]':
    ...
    """
    return self.model_copy(update={
      'group_fields': [*self.group_fields, *fields]
    })
  ...
  def join(self, join_clause: str) -> 'Query[T]':
    ...
    """
    return self.model_copy(update={
      'join_clauses': [*self.join_clauses, join_clause]
    })
...
def from_table(query: Query[T], table: str) -> Query[T]:
  """Add table to query.
...
def where(query: Query[T], condition: str | Operator) -> Query[T]:
  """Add WHERE condition to query.
...
def order_by(query: Query[T], field: str, direction: str = 'ASC') -> Query[T]:
...
def limit(query: Query[T], n: int) -> Query[T]:
...
def offset(query: Query[T], n: int) -> Query[T]:
