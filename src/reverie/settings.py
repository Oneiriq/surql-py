...
This module provides configuration management for reverie with support for
multiple configuration sources in priority order:

1. Environment variables (REVERIE_* prefix)
2. .env file
3. pyproject.toml [tool.reverie] section
4. Default values
...
class PyProjectTomlSettingsSource(PydanticBaseSettingsSource):
  """Custom settings source that reads from pyproject.toml [tool.reverie] section."""
    ...
    if pyproject_path.exists():
      try:
        with open(pyproject_path, 'rb') as f:
          data = tomllib.load(f)
        self._toml_data = data.get('tool', {}).get('reverie', {})
...
class Settings(BaseSettings):
  ...
  Example:
    ...
    pyproject.toml: [tool.reverie] migration_path = "db/migrations"
  """
  ...
  def settings_customise_sources(
    ...
    4. pyproject_toml - pyproject.toml [tool.reverie]
