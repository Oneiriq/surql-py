...
from src.migration.discovery import discover_migrations, validate_migration_name
...
from src.migration.models import MigrationDirection, MigrationState
...
@app.command('up')
def migrate_up(
  directory: Annotated[Optional[Path], directory_option] = None,
  steps: Annotated[
    Optional[int],
    typer.Option('--steps', '-n', help='Number of migrations to apply (default: all)')
  ] = None,
...
def migrate_down(
  ...
  steps: Annotated[
    int,
    typer.Option('--steps', '-n', help='Number of migrations to rollback')
  ] = 1,
  dry_run: Annotated[bool, dry_run_option] = False,
  verbose: Annotated[bool, verbose_option] = False,
) -> None:
...
def create_migration(
  ...
  try:
    ...
    display_info(f'Edit the file to add your migration SQL')
