...
async def execute_migration(
  ...
  try:
    ...
    # Run statements individually so per-statement errors surface as raised
    # `QueryError`s — see issue #83. The previous batched
    # `BEGIN TRANSACTION; <stmts>; COMMIT TRANSACTION;` path was atomic but
    # silently swallowed every per-statement failure: the surrealdb-py SDK
    # returns `null` for a failed batched transaction (the per-statement
    # error list never reaches the caller), and surql-py only caught raised
    # exceptions — so `migrate_up` would write the history row, log
    # `applied_count=1`, and leave zero of the migration's tables defined.
    #
    # Trade-off: a mid-migration failure now leaves earlier statements
    # applied (no rollback). For an additive migration generated by
    # `generate_initial_migration` the resulting state is incomplete but
    # recoverable — fix the offending statement, drop the partial schema,
    # and re-run. The visibility win matters far more than the atomicity
    # loss for the failure case, and the success case is unchanged.
