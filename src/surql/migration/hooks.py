...
This module provides utilities for integrating surql schema drift detection
...
async def check_schema_drift(
  ...
  if fail_on_drift:
    return HookCheckResult(
      ...
      suggested_action=(
        "Generate a migration with: surql schema generate -s <schema-file> -m '<description>'"
      ),
    )
  ...
  return HookCheckResult(
    ...
    suggested_action=(
      ...
      "surql schema generate -s <schema-file> -m '<description>'"
    ),
  )
...
def generate_precommit_config(
  ...
  """Generate .pre-commit-config.yaml entry for surql schema check.
...
def _detect_code_drift(
  ...
  use the database-connected drift check via `surql schema validate`.
  ...
  for schema_file in schema_files:
    try:
      ...
      drift_descriptions = [
        ...
        'Run "surql schema validate" for detailed drift detection',
      ]
