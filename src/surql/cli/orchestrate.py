...
from surql.orchestration.config import configure_environments, get_registry
from surql.orchestration.coordinator import MigrationCoordinator
from surql.orchestration.health import HealthCheck
...
def deploy_command(
  ...
  Examples:
    ...
    Rolling deployment with batches of 2:
      surql orchestrate deploy -e prod1,prod2,prod3,prod4 --strategy rolling --batch-size 2
...
def _display_deployment_results(results: dict[str, DeploymentResult]) -> None:
  ...
  """
  from surql.orchestration.strategy import DeploymentResult
