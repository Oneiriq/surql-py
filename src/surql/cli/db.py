...
from surql.migration.history import create_migration_table
...
async def _reset_database_async(skip_confirm: bool, verbose: bool) -> None:
  ...
  try:
    async with get_client(config) as client:
      ...
      display_info('Run "surql db init" to reinitialize migration tracking')
...
def execute_query(
  ...
  Examples:
    Execute query:
    $ surql db query "SELECT * FROM user LIMIT 5"

    Execute with table format:
    $ surql db query "SELECT * FROM user" --format table
  """
  try:
